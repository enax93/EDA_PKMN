...
def diagrama_dispersion(df, col_x, col_y, col_size, col_hue=None, escala = 1,mostrar_leyenda=False,barra_color=False):
    """
    Crea un scatter plot usando dos columnas para los ejes X e Y,
    una tercera columna para determinar el tamaño de los puntos, 
    y una cuarta columna para determinar el color de los puntos.
    ...
    col_y (str): Nombre de la columna para el eje Y.
    col_size (str): Nombre de la columna para determinar el tamaño de los puntos.
    col_hue(str): Nombre de la columna para determinar el color de los puntos.
    """
    plt.figure(figsize=(20,8))

    # Asegúrate de que los valores de tamaño sean positivos
    sizes = (df[col_size] - df[col_size].min() + 1)* escala

    if col_hue is None:
        plt.scatter(df[col_x], df[col_y],s=sizes, alpha=0.5)
    else:    
        plt.scatter(df[col_x], df[col_y], c=df[col_hue],s=sizes,alpha=0.5)
    ...
    if mostrar_leyenda:
        for sizes in [100,200,300,400,500,600]:
            plt.scatter([],[],c = "k", alpha = 0.7, s =  sizes, label = str(sizes) + "Km$^2$")
            plt.legend(frameon = False, title = "Area Ciudad", ncol=3)
    
    if col_hue:
        plt.title(f'Diagrama de dispersión {col_x} vs {col_y} con color basado en {col_hue} y tamaño basado en {col_size}')
    else:
        plt.title(f'Diagrama de dispersión {col_x} vs {col_y} y tamaño basado en {col_size}')
